...
//data inicial
    const dataInicial = {
        loading:false, //sirve para desabilitar botones cuando el usuario se esté logeando
        activo:false //usuario activo o no
    }
...
export const ingresoUsuarioAccion = () => async(dispatch) => {
    ...
    try {
        ...
        const usuarioDB = await db.collection('usuarios').doc(usuario.email).get(usuario)
        console.log(usuarioDB)
        if(usuarioDB.exists){
            //cuando existe el usuario en firebase
            dispatch({
                type: USUARIO_EXITO,
